...
/**
 * GirlModelCard - A specialized card component for girl models with consistent styling
 * 
...
const GirlModelCard = ({ 
  ...
  const internalBioUrl = `/girls/model/${slug || `model-${index}`}`;
  ...
  const whitelabelUrl = `https://awejmp.com/?siteId=wl3&cobrandId=201300&performerName=${slug}&prm[psid]=mikeeyy3&prm[pstool]=213_1&prm[psProgram]=cbrnd&categoryName=girl&pageName=freechat`;
  ...
  console.log(`[GirlModelCard ${index}] Rendering model: ${name}, image: ${imageUrl.substring(0, 50)}...`);
      ...
      <a href={whitelabelUrl} target="_blank" rel="noopener noreferrer" className="block">
        <div className="model-card-image-container">
          <Image
            ...
            priority={index < 10}
            unoptimized={true}
            onLoad={() => console.log(`[GirlModelCard ${index}] Image loaded: ${name}`)}
            onError={() => console.error(`[GirlModelCard ${index}] Error loading image: ${imageUrl}`)}
          />
