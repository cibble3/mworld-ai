...
// Trans Model Profile Page
const TransModelProfile = ({ model, similar }) => {
  ...
  useEffect(() => {
    if (model?.isOnline && chatContainerRef.current) {
      ...
      script.src = `https://ttedwm.com/embed/lfcht?c=object_container&site=wl3&cobrandId=201300&psid=mikeeyy3&pstool=320_1&psprogram=cbrnd&campaign_id=117404&category=trans&forcedPerformers[]=${model.slug}&vp[showChat]=true&vp[chatAutoHide]=&vp[showCallToAction]=true&vp[showPerformerName]=true&vp[showPerformerStatus]=true&ctaLabelKey=udmn&landingTarget=freechat&ms_notrack=1&categoryName=transgender&pageName=freechat`;
  ...
  if (!model) {
    return (
      <div className="min-h-screen bg-[#16181c] text-white flex flex-col items-center justify-center p-4">
        ...
        <Link href="/trans" className="bg-[#E0006C] text-white px-6 py-2 rounded-md">
          Return to Trans Cams
        </Link>
      </div>
    );
  }
  ...
  const whitelabelUrl = `https://awejmp.com/?siteId=wl3&cobrandId=201300&performerName=${model.slug}&prm[psid]=mikeeyy3&prm[pstool]=213_1&prm[psProgram]=cbrnd&categoryName=transgender&pageName=freechat`;
  
  return (
    <>
      <Head>
        <title>{model.name} - Trans Cam Model | MistressWorld</title>
        <meta name="description" content={`Chat live with ${model.name}, ${model.age} year old ${model.ethnicity || ''} trans cam model. Explore ${model.name}'s specialties including ${(model.tags || []).slice(0, 5).join(', ')}.`} />
        <meta property="og:title" content={`${model.name} - Trans Cam Model | MistressWorld`} />
        <meta property="og:description" content={`Chat live with ${model.name}, ${model.age} year old trans cam model on MistressWorld.`} />
      ...
      <div className="min-h-screen bg-[#16181c] text-white">
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="bg-[#1a1c21] rounded-lg overflow-hidden shadow-lg p-6 mb-8">
            ...
            <p className="mb-4">
              Welcome to {model.name}'s profile on MistressWorld.xxx. {model.name} is a {model.age}-year-old {model.ethnicity || ''} trans cam model 
            ...
            <p className="mb-4">
              ...
              {model.name} has become one of the most sought-after models in our trans cam collection.
            </p>
            <p>
          ...
          {similar && similar.length > 0 && (
            <div className="mb-8">
              <h2 className="text-2xl font-bold mb-4">Similar Trans Models</h2>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {similar.map((model, i) => (
                  <div key={i} className="bg-[#1a1c21] rounded-lg overflow-hidden shadow-lg">
                    <Link href={`/trans/model/${model.slug}`} className="block">
                    ...
                    <div className="px-3 pb-3">
                      <a 
                        href={`https://awejmp.com/?siteId=wl3&cobrandId=201300&performerName=${model.slug}&prm[psid]=mikeeyy3&prm[pstool]=213_1&prm[psProgram]=cbrnd&categoryName=transgender&pageName=freechat`} 
                        target="_blank" 
                        rel="noopener noreferrer"
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    console.log(`[getServerSideProps trans/model/[id].js] Fetching model data for id: ${id}`);
    
    // Construct fetch URL for model search
    const modelSearchUrl = `${baseUrl}/api/models/trans?limit=32&_timestamp=${Date.now()}`;
    ...
    console.error(`[getServerSideProps trans/model/[id].js] Error:`, error.message);
    return {
      notFound: true,
    };
  }
...
export default TransModelProfile; 