...
console.log('[API] /api/models/fetish.js module loaded');

// Run a test fetch on module load
(async () => {
  try {
    console.log('[API] Testing direct fetish models API call on module load');
    ...
    console.log(`[API] Test fetish models API URL: ${url.toString()}`);
    ...
    try {
      ...
      console.log(`[API] Test fetish API response status: ${testResponse.status}, has data: ${!!testResponse.data}`);
      if (testResponse.data?.data?.models?.length > 0) {
        console.log(`[API] Found ${testResponse.data.data.models.length} test models`);
      }
    } catch (err) {
      console.error(`[API] Test direct fetish API call failed: ${err.message}`);
    }
  } catch (err) {
    console.error(`[API] Error testing direct fetish API: ${err.message}`);
  }
})();

// AWE API Config - Specific for fetish models
const AWE_CONFIG = {
...
export default async function handler(req, res) {
  ...
  // Set up the API params for AWE Fetish models
  const params = {
    ...
    category: 'fetish', // Specific to fetish models
    limit: parsedLimit,
    offset: parsedOffset,
  ...
  console.log(`[API /models/fetish] Fetching fetish models with params:`, params);
  
  // Log the full URL that will be used
  const fullUrl = getFullUrl(params);
  console.log(`[API /models/fetish] Full request URL: ${fullUrl}`);

  try {
    // Make the API request with direct URL for easier debugging
    console.log(`[API /models/fetish] Making direct request to: ${fullUrl}`);
    ...
    console.log(`[API /models/fetish] Response status: ${response.status}`);
    console.log(`[API /models/fetish] Response headers:`, response.headers);
    
    // Check if we have data
    if (!response.data || !response.data.data || !response.data.data.models) {
      console.error(`[API /models/fetish] Invalid response structure:`, 
    ...
    console.log(`[API /models/fetish] Fetched ${normalizedModels.length} fetish models`);
    ...
    console.error(`[API /models/fetish] Error fetching fetish models:`, error.message);
    
    // For development, provide hardcoded data when API fails
    if (process.env.NODE_ENV !== 'production') {
      console.log(`[API /models/fetish] Returning hardcoded data for development`);
      
      // Create some realistic fetish model data
      const hardcodedModels = Array(12).fill().map((_, i) => ({
        id: `fetish-model-${i+1}`,
        slug: `fetish-model-${i+1}`,
        name: `Mistress ${['Victoria', 'Domina', 'Kinkster', 'Leather', 'Latex', 'Queen'][i % 6]} ${i+1}`,
        thumbnail: `https://loremflickr.com/320/180/model?lock=${i+100}`, // Fixed 320x180 aspect ratio
        preview: `https://loremflickr.com/320/180/model?lock=${i+100}`, // Same image for consistency
    ...
    return res.status(500).json({
      success: false,
      error: `Failed to fetch fetish models: ${error.message}`,
      data: {
        models: [],
        pagination: {
          total: 0,
