...
console.log('[API] /api/models/girls.js module loaded');

// Run a test fetch on module load
(async () => {
  try {
    console.log('[API] Testing direct girls models API call on module load');
    ...
    const testParams = {
      ...
      psProgram: 'cbrnd',
      campaignId: '117404',
      category: 'girl', // Changed from 'fetish' to 'girl'
      limit: 1,
      imageSizes: '320x180',
      imageType: 'ex',
      showOffline: 0,
    ...
    console.log(`[API] Test girls models API URL: ${url.toString()}`);
    ...
    try {
      ...
      console.log(`[API] Test girls API response status: ${testResponse.status}, has data: ${!!testResponse.data}`);
      if (testResponse.data?.data?.models?.length > 0) {
        console.log(`[API] Found ${testResponse.data.data.models.length} test models`);
      }
    } catch (err) {
      console.error(`[API] Test direct girls API call failed: ${err.message}`);
    }
  } catch (err) {
    console.error(`[API] Error testing direct girls API: ${err.message}`);
  }
})();

// AWE API Config - Specific for girls models
const AWE_CONFIG = {
...
export default async function handler(req, res) {
  ...
  // Set up the API params for AWE Girls models
  const params = {
  ...
  console.log(`[API /models/girls] Fetching girls models with params:`, params);
  
  // Log the full URL that will be used
  const fullUrl = getFullUrl(params);
  console.log(`[API /models/girls] Full request URL: ${fullUrl}`);

  try {
    // Make the API request with direct URL for easier debugging
    console.log(`[API /models/girls] Making direct request to: ${fullUrl}`);
    ...
    console.log(`[API /models/girls] Response status: ${response.status}`);
    console.log(`[API /models/girls] Response headers:`, response.headers);
    
    // Check if we have data
    if (!response.data || !response.data.data || !response.data.data.models) {
      console.error(`[API /models/girls] Invalid response structure:`, 
    ...
    console.log(`[API /models/girls] Fetched ${normalizedModels.length} girls models`);
    ...
    console.error(`[API /models/girls] Error fetching girls models:`, error.message);
    
    // For development, provide hardcoded data when API fails
    if (process.env.NODE_ENV !== 'production') {
      console.log(`[API /models/girls] Returning hardcoded data for development`);
      
      // Create some realistic girls model data
      const hardcodedModels = Array(12).fill().map((_, i) => ({
        id: `girl-model-${i+1}`,
        slug: `girl-model-${i+1}`,
        name: `${['Sophia', 'Emma', 'Isabella', 'Olivia', 'Charlotte', 'Mia'][i % 6]} ${i+1}`,
    ...
    return res.status(500).json({
      success: false,
      error: `Failed to fetch girls models: ${error.message}`,
      data: {
        models: [],
        pagination: {
          total: 0,
